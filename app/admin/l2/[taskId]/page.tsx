...
export default function L2TaskDetailPage() {
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    ...
    // Section 5.2: In L2 submission, strictly scope to mutable decision variable(s)
    const variables: Record<string, unknown> = {
      l2_decision: String(values.l2_decision || "").toLowerCase().trim(),
    };
    if (values.l2_comment && typeof values.l2_comment === "string" && values.l2_comment.trim()) {
      variables.l2_comment = values.l2_comment.trim();
    }
    ...
    try {
      ...
      const json = await res.json();
      if (!json.success) {
        const errorMsg = json.error?.message || "Failed to submit final decision.";
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-gray-200/80 rounded-2xl p-6 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-xl font-bold text-gray-900 mt-1">
            Final Approval for Visitor Pass
          </h1>
          <p className="text-xs text-gray-500">
            Form Contract: <span className="font-mono">l2-form</span> &bull; HumanTask: PlanItem_0eptewu
          </p>
        </div>
      </div>
      ...
      {completedSuccess && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3 text-emerald-800 text-sm animate-fade-in">
          ...
          <div>
            <p className="font-semibold">L2 Decision Submitted Successfully!</p>
            <p className="text-xs text-emerald-700">
              CMMN milestone / terminal state evaluated. Redirecting to L2 queue...
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-gray-200/80 rounded-2xl p-6 sm:p-8 shadow-xs">
        {isLoading ? (
          <div className="py-16 text-center flex flex-col items-center justify-center">
            ...
            <p className="text-sm font-medium text-gray-700">Loading task schema and visitor audit history...</p>
          </div>
        ) : schema ? (
          <DynamicForm
            schema={schema}
